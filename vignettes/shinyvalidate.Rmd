...
The first rule you'll need to add for each input is either [`sv_required()`](https://rstudio.github.io/shinyvalidate/reference/sv_required.html) or [`sv_optional()`](https://rstudio.github.io/shinyvalidate/reference/sv_optional.html).
...
The `sv_optional()` helper is used when an input is not required, but when it is provided, it needs to be validated by subsequent rules. For example, if an email is not required:
...
If `sv_optional()` detects that an input was not provided, it causes remaining tests for that input to be skipped. In the example above, if the user does not provide a value for `input$email`, the input will be considered valid; but if they do provide an `input$email`, the input will be checked against `is_valid_email()` to determine validity.

<!-- TODO: change the above when `sv_valid_email()` is available -->

Because `sv_optional()` only causes _subsequent_ rules to be skipped, order matters; be sure to add `sv_optional()` before any other rules for the same input.

### Rules from helper functions

shinyvalidate comes with a few helper functions that implement common validation rules.
